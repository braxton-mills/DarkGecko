...
class DocumentProcessor implements IDocumentProcessor {
  ...
  protected readonly _standardPseudoCssTexts = new Map<
    PseudoStyleStandard,
    string
  >();
  ...
  protected readonly _filterForFilterProcessing =
    this.getFilterOfElementsForFilterProcessing();

  protected get shift() {
    return this._settingsManager.shift;
  }
  ...
  constructor(
    ...
    this._documentObserver.onUpdateChanged.addListener(
      this.onDocumentUpdateStageChanged as any,
      this,
    );
  }
  ...
  private addListeners() {
    dom.addEventListener(
      this._rootDocument,
      'DOMContentLoaded',
      this.onDocumentContentLoaded,
      this,
    );
    this._settingsManager.onSettingsInitialized.addListener(
      this.onSettingsInitialized,
      this,
    );
    this._settingsManager.onSettingsChanged.addListener(
      this.onSettingsChanged,
      this,
      EventHandlerPriority.Low,
    );
    this._documentObserver.onElementsAdded.addListener(
      this.onElementsAdded as any,
      this,
    );
    this._documentObserver.onClassChanged.addListener(
      this.onClassChanged as any,
      this,
    );
    this._documentObserver.onStyleChanged.addListener(
      this.onStyleChanged as any,
      this,
    );
    this._styleSheetProcessor.onElementsForUserActionObservationFound.addListener(
      this.onElementsForUserActionObservationFound as any,
      this,
    );
  }
  ...
  protected createStandardPseudoCssTexts() {
    const invertedBackgroundImageFilter = [
        this.shift.BackgroundImage.saturationLimit < 1
          ? `saturate(${this.shift.BackgroundImage.saturationLimit})`
          : '',
        `brightness(${float.format(
          1 - this.shift.Background.lightnessLimit,
        )}) hue-rotate(180deg) invert(1)`,
        this._settingsManager.currentSettings.blueFilter !== 0
          ? `var(--${FilterType.BlueFilter})`
          : '',
      ]
        .filter((f) => f)
        .join(' ')
      ...
      backgroundImageFilter = [
        this.shift.BackgroundImage.saturationLimit < 1
          ? `saturate(${this.shift.BackgroundImage.saturationLimit})`
          : '',
        this.shift.BackgroundImage.lightnessLimit < 1
          ? `brightness(${this.shift.BackgroundImage.lightnessLimit})`
          : '',
        this._settingsManager.currentSettings.blueFilter !== 0
          ? `var(--${FilterType.BlueFilter})`
          : '',
    ...
    this._standardPseudoCssTexts.set(
      ...
      `${this._css.filter}:${invertedBackgroundImageFilter}!${this._css.important}`,
    );
    this._standardPseudoCssTexts.set(
      PseudoStyleStandard.BackgroundImage,
      `${this._css.filter}:${backgroundImageFilter}!${this._css.important}`,
    );
  }

  protected onSettingsChanged(
    response: (scheme: ColorScheme) => void,
  ...
  protected onSettingsInitialized(): void {
    if (this._settingsManager.isActive) {
      ...
      this.injectDynamicValues(this._rootDocument);
      if (
        this._rootDocumentContentLoaded ||
        this._rootDocument.readyState === 'complete'
      ) {
        this._rootDocumentContentLoaded = true;
        this.processRootDocument();
      } else {
        this.setDocumentProcessingStage(
          this._rootDocument,
      ...
      if (
        ...
        if (
          ...
          this._settingsManager.onSettingsChanged.addListener(
            (response, shift) => {
              throw new Error(
                'Midnight Lizard has been updated. Please refresh the page.',
              );
            },
            this,
          );
        ...
        html.setAttribute(
          'ml-view',
          window.top === window.self ? 'top' : 'child',
        );
        if (this._settingsManager.isActive) {
          if (
            this._rootDocument.body &&
            this._rootDocument.body.childElementCount === 1
          ) {
            if (
              this._rootDocument.body.firstElementChild instanceof
              HTMLEmbedElement
            ) {
          ...
          html.setAttribute(
            'ml-scrollbar-style',
            this._settingsManager.currentSettings.scrollbarStyle
              ? 'ml-simple'
              : 'original',
          );
  ...
  protected getLastDoccumentProcessingMode(doc: Document) {
    return (
      (doc.documentElement!.getAttribute('ml-mode') as ProcessingMode) ||
    ...
    if (doc.body && doc.defaultView && this._settingsManager.isActive) {
      ...
      if (this._settingsManager.isComplex) {
        ...
        let allTags = Array.from(doc.body.getElementsByTagName('*'))
          ...
          .filter(
            this.getFilterOfElementsForComplexProcessing(),
          ) as HTMLElement[];
        ...
        this._documentObserver.startDocumentObservation(doc);
        let allTags = Array.from(doc.body.getElementsByTagName('*')).filter(
          this.getFilterOfElementsForComplexProcessing(),
        ) as HTMLElement[];
        DocumentProcessor.processAllElements(
          allTags,
          this,
          smallReCalculationDelays,
        );
      } else if (
        this._settingsManager.isFilter &&
  ...
  private getFilterOfElementsForComplexProcessing(): (
    value: Element,
  ) => boolean {
    return (tag) =>
      this.checkElement(tag) &&
      (!!tag.parentElement || tag === tag.ownerDocument!.documentElement);
  }

  private getFilterOfElementsForFilterProcessing(): (
    value: Element,
  ) => boolean {
    return (tag) => {
      if (
        ...
        tag.mlComputedStyle =
          tag.mlComputedStyle ||
          tag.ownerDocument!.defaultView!.getComputedStyle(tag, '');
        if (
          this._backgroundColorProcessor.isDark(
            tag.mlComputedStyle.backgroundColor,
          )
        ) {
          tag.mlInvert = true;
        }
  ...
  private getFilterOfElementsForSimplifiedProcessing(): (
    value: Element,
  ) => boolean {
    return (tag) => {
      if (
        ...
        if (
          ...
          (tag instanceof HTMLEmbedElement &&
            tag.getAttribute('type') === 'application/pdf')
        ) {
          return true;
        }
        tag.mlComputedStyle =
  ...
  private calculateDefaultColors(
    doc: Document,
    ...
    if (!defaultLinkColor) {
      ...
      if (aWithoutClass) {
        defaultLinkColor =
          doc.defaultView!.getComputedStyle(aWithoutClass).color!;
      }
    }
    if (!defaultTextColor) {
      defaultTextColor = doc.defaultView!.getComputedStyle(doc.body).color!;
    }
    defaultLinkColor = this._linkColorProcessor.calculateDefaultColor(
      doc,
      defaultLinkColor,
    );
    this._visitedLinkColorProcessor.calculateDefaultColor(
      doc,
      defaultLinkColor,
    );
    ...
    this._activeVisitedLinkColorProcessor.calculateDefaultColor(
      doc,
      defaultLinkColor,
    );
    this._hoverVisitedLinkColorProcessor.calculateDefaultColor(
      doc,
      defaultLinkColor,
    );
  ...
  protected hasPseudoClassOverrided(tag: HTMLElement, pseudo: PseudoClass) {
    return (
      !!tag.mlComputedStyle &&
      tag.mlComputedStyle.getPropertyValue(
        `--ml-pseudo-${PseudoClass[pseudo].toLowerCase()}`,
      ) === true.toString()
    );
  }

  protected onElementsForUserActionObservationFound([pseudoClass, tags]: [
    PseudoClass,
    NodeListOf<Element>,
  ]) {
  ...
  protected observeUserActions(tag: HTMLElement) {
    ...
    const preFilteredSelectors =
      this._styleSheetProcessor.getPreFilteredSelectors(tag);
    if (preFilteredSelectors.length > 0) {
      hover =
        hover ||
        this._styleSheetProcessor.canHavePseudoClass(
          tag,
          preFilteredSelectors,
          PseudoClass.Hover,
        );
      focus =
        focus ||
  ...
  protected observeCheckedUserAction(tag: Element) {
    if (tag instanceof HTMLInputElement) {
      ...
      const checkBox = tag.ownerDocument!.getElementById(
        tag.htmlFor,
      ) as HTMLInputElement;
      if (checkBox) {
        ...
        dom.addEventListener(
          checkBox,
          'change',
          this._boundCheckedLabelHandler,
        );
      }
    }
  }
    ...
    if (this._settingsManager.isActive && this._settingsManager.isComplex) {
      ...
      const eventTargets =
        tag instanceof HTMLTableCellElement
          ? (Array.from(tag.parentElement!.children) as HTMLElement[])
          : [tag];

      this.reCalcAllRootElements(
        new Set(
          eventTargets.filter(
            (target) =>
  ...
  private reCalcAllRootElements(
    rootElements: Set<HTMLElement>,
    andAllChildren: boolean,
    skipSelectors = false,
  ) {
    if (rootElements && rootElements.size) {
      const allElements = Array.from(rootElements).reduce(
        (allElems, rootElement) =>
          allElems.concat(
            this.reCalcRootElement(rootElement, andAllChildren, skipSelectors),
          ),
        new Array<HTMLElement>(),
      );
      if (allElements.length) {
        this._documentObserver.stopDocumentObservation(
          allElements[0].ownerDocument!,
        );
        allElements.forEach((tag) => this.restoreElementColors(tag, true));
        this._documentObserver.startDocumentObservation(
          allElements[0].ownerDocument!,
        );
        DocumentProcessor.processAllElements(
          allElements,
          this,
          andAllChildren
            ? onCopyReCalculationDelays
            : allElements.length < 50
            ? smallReCalculationDelays
            : bigReCalculationDelays,
          true,
          false,
    ...
    if (
      ...
      if (allTags && allTags.length > 0) {
        skipSelectors =
          ...
          !this._settingsManager.isComplex ||
          andAllChildren ||
          this._styleSheetProcessor.getSelectorsQuality(
            rootElem.ownerDocument!,
          ) === 0 ||
        ...
        let filteredTags = allTags.filter((el) => {
          ...
          if (!skipSelectors) {
            const newSelectors =
              this._styleSheetProcessor.getElementMatchedSelectors(el);
  ...
  protected onStyleChanged(changedElements: Set<HTMLElement>) {
    ...
    changedElements.forEach((tag) => {
      let needReCalculation = !!tag.mlVisualAttributeChanged,
      ...
      if (
        (value &&
          tag.style.getPropertyPriority(ns.css.bgrColor) !==
            this._css.important) ||
      ...
      if (
        (value &&
          tag.style.getPropertyPriority(ns.css.fntColor) !==
            this._css.important) ||
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.textShadow) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(ns.css.brdColor) !== this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.borderTopColor) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.borderRightColor) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.borderBottomColor) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.borderLeftColor) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.backgroundImage) !==
          this._css.important
      ) {
      ...
      if (
        value &&
        tag.style.getPropertyPriority(this._css.transitionProperty) !==
          this._css.important
      ) {
  ...
  protected onElementsAdded(addedElements: Set<HTMLElement>) {
    ...
    const allNewTags = Array.from(addedElements.values()).filter(
      (x) => !this.IsIgnored(x) && this.checkElement(x),
    );
    const allChildTags = new Set<HTMLElement>();
    allNewTags.forEach((newTag) => {
      if (newTag.getElementsByTagName) {
        Array.prototype.forEach.call(
          newTag.getElementsByTagName('*'),
          (childTag: HTMLElement) => {
    ...
    DocumentProcessor.processAllElements(
      allNewTags.concat(Array.from(allChildTags.values())),
      this,
      this._settingsManager.isComplex
        ? bigReCalculationDelays
        : smallReCalculationDelays,
    ...
    if (allTags.length > 0) {
      ...
      for (let tag of allTags) {
        ...
        if (
          isVisible ||
          tag.mlComputedStyle ||
          !delayInvisibleElements ||
          allTags.length < minChunkableLength
        ) {
          tag.mlComputedStyle =
            tag.mlComputedStyle || doc.defaultView!.getComputedStyle(tag, '');
          ...
          hasBgImage =
            !hasBgColor &&
            tag.mlComputedStyle!.backgroundImage !== docProc._css.none;
        }
        ...
        if (isVisible) {
          ...
          isVisible &&
            (tag.mlArea = tag.mlArea || tag.mlRect.width * tag.mlRect.height);
          inView =
            isVisible &&
            ((tag.mlRect.bottom >= 0 && tag.mlRect.bottom <= hm) ||
              (tag.mlRect.top >= 0 && tag.mlRect.top <= hm)) &&
            ((tag.mlRect.right >= 0 && tag.mlRect.right <= wm) ||
              (tag.mlRect.left >= 0 && tag.mlRect.left <= wm));
          if (!isVisible) {
      ...
      allTags.sort((a, b) =>
        a instanceof HTMLBodyElement
          ? -9999999999
          : b instanceof HTMLBodyElement
          ? 9999999999
          : a.mlOrder !== b.mlOrder
          ? a.mlOrder! - b.mlOrder!
          : b.mlArea && a.mlArea && b.mlArea !== a.mlArea
          ? b.mlArea - a.mlArea
          : a.mlRowNumber! - b.mlRowNumber!,
      );

      otherInvisTags = otherInvisTags.filter(
        (tag) => tag.mlOrder === po.delayedInvisTags,
      );
      if (otherInvisTags.length && otherInvisTags.length < allTags.length) {
        // removing invisible elements
        allTags.splice(
          allTags.length - otherInvisTags.length,
          otherInvisTags.length,
        );
      }
      ...
      const results = handlePromise(
        DocumentProcessor.processOrderedElements(
          allTags,
          docProc,
          delays,
          needObservation,
      ...
      if (otherInvisTags.length > 0) {
        Promise.all([otherInvisTags, docProc, delays, results]).then(
          ([otherTags, dp, dl]) => {
            ...
            forEachPromise(
              sliceIntoChunks(otherTags, chunkLength * 2).map((chunk) => [
                chunk,
                dp,
                dl,
              ]),
  ...
  private static fixParentElementsOrder(
    parentElement: HTMLElement,
    procOrd: ProcessingOrder,
  ): any {
  ...
  protected static fixColorInheritance(
    allTags: HTMLElement[],
    docProc: DocumentProcessor,
    results: Promise<any>,
  ) {
    if (docProc._settingsManager.isComplex) {
      ...
      waitResults.then(([tags, dp]) => {
        if (tags && tags.length > 0) {
          setTimeout(
            (t: HTMLElement[], dProc: DocumentProcessor) => {
              const brokenColorTags = t.filter(
                (tag: HTMLElement) =>
                  ...
                  tag.mlColor.intendedColor &&
                  tag.mlComputedStyle &&
                  tag.mlColor.intendedColor !==
                    (tag instanceof HTMLElement
                      ? (tag as any).mlComputedStyle!.color
                      : (tag as any).mlComputedStyle!.fill),
              );

              if (brokenColorTags.length > 0) {
                dProc._documentObserver.stopDocumentObservation(
                  brokenColorTags[0].ownerDocument!,
                );
                brokenColorTags.forEach((tag) => {
                  ...
                  tag.originalColor = tag.style.getPropertyValue(
                    ns.css.fntColor,
                  );
                  tag.style.setProperty(
                    ns.css.fntColor,
                    newColor.color,
                    dProc._css.important,
                  );
                });
                docProc._documentObserver.startDocumentObservation(
                  brokenColorTags[0].ownerDocument!,
                );
              }
            },
      ...
      waitResults.then(([tags, dp]) => {
        if (tags && tags.length > 0) {
          setTimeout(
            (t: HTMLElement[], dProc: DocumentProcessor) => {
              const brokenTransparentTags = t.filter(
                (tag) =>
                  tag.ownerDocument!.defaultView &&
                  tag.mlBgColor &&
                  !tag.mlBgColor.color &&
              ...
              if (brokenTransparentTags.length > 0) {
                dProc._documentObserver.stopDocumentObservation(
                  brokenTransparentTags[0].ownerDocument!,
                );
                brokenTransparentTags.forEach((tag) => {
                  dProc.restoreElementColors(tag, true);
                  tag.mlFixed = 'bgcolor';
                });
                dProc._documentObserver.startDocumentObservation(
                  brokenTransparentTags[0].ownerDocument!,
                );
                DocumentProcessor.processAllElements(
                  brokenTransparentTags,
                  dProc,
                  bigReCalculationDelays,
                );
              }
  ...
  private static calculateNewRoomRulesToFixColorInheritanceInPrevChunk(
    ...
    return prevChunk
      ...
      .map((tag) => {
        ...
        return {
          tag: tag,
          result: { roomRules: { color: newColor } },
        };
      });
  }

  protected static processOrderedElements(
    tags: HTMLElement[],
    docProc: DocumentProcessor,
    delays = normalDelays,
    needObservation = false,
  ) {
    if (tags.length > 0) {
      const density = 2000 / tags.length;
      needObservation =
        needObservation &&
        docProc._settingsManager.isComplex &&
        !!docProc._styleSheetProcessor.getSelectorsCount(
          tags[0].ownerDocument!,
        );
      let result: Promise<HTMLElement[]>;
      if (tags.length < minChunkableLength) {
        result = DocumentProcessor.processElementsChunk(
          tags,
          docProc,
          null,
        ...
        result = forEachPromise(
          this.concatZeroDelayedChunks(
            sliceIntoChunks(tags, chunkLength),
            getNextDelay as any,
          ).map((chunk) => [chunk, docProc]),
          DocumentProcessor.processElementsChunk,
          0,
          getNextDelay as any,
        );
      }
      if (needObservation) {
        Promise.all([
          ...
          ...docProc._styleSheetProcessor.getCssPromises(
            tags[0].ownerDocument!,
          )!,
  ...
  protected static processElementsChunk(
    chunk: HTMLElement[],
    docProc: DocumentProcessor,
    prevChunk?: HTMLElement[] | null,
    delay?: number,
  ) {
    docProc._documentObserver.stopDocumentObservation(chunk[0].ownerDocument!);

    const paramsForPromiseAll: any[] = [
      [...chunk],
      chunk[0].ownerDocument!,
      delay,
    ];
    if (prevChunk) {
      const brokenTransparentElements =
        DocumentProcessor.getBrokenTransparentElements(prevChunk);
    ...
    const results =
      DocumentProcessor.calculateNewRoomRulesToFixColorInheritanceInPrevChunk(
        prevChunk || [],
        docProc,
      ).concat(
        chunk.map((tag) => ({
          tag: tag,
          result: docProc._settingsManager.isComplex
    ...
    results
      ...
      .map((tr) => {
        return Object.assign(tr, {
          beforeRoomRules:
            tr.result!.before && docProc.calculateRoomRules(tr.result!.before!),
          afterRoomRules:
            tr.result!.after && docProc.calculateRoomRules(tr.result!.after!),
        });
      })
      .map((tr) => {
        docProc.applyRoomRules(tr.tag, tr.result!.roomRules);
        if (tr.beforeRoomRules) {
          docProc.applyRoomRules(
            tr.result!.before!,
            tr.beforeRoomRules.roomRules,
          );
        }
        if (tr.afterRoomRules) {
          docProc.applyRoomRules(
            tr.result!.after!,
            tr.afterRoomRules.roomRules,
          );
        }
        return [tr.result!.before, tr.result!.after]
          .filter((x) => x)
          .map((x) => x!.stylePromise);
      })
      .filter((r) => r)
      .forEach((r) => paramsForPromiseAll.push(...r!.map(handlePromise)));

    docProc._documentObserver.startDocumentObservation(chunk[0].ownerDocument!);

    return Promise.all(
      paramsForPromiseAll as [
        HTMLElement[],
        Document,
        number,
  ...
  protected static startObservation(
    tags: HTMLElement[],
    docProc: DocumentProcessor,
  ) {
    tags.forEach((tag) => {
  ...
  protected tagIsSmall(tag: Element | PseudoElement): boolean {
    ...
    let maxSize = this._settingsManager.isFilter ? 100 : 50,
      maxAxis = this._settingsManager.isFilter ? 50 : 25,
      check = (w: number, h: number) =>
        w > 0 &&
        h > 0 &&
        ((w < maxSize && h < maxSize) || w < maxAxis || h < maxAxis);
    tag.mlComputedStyle =
      tag.mlComputedStyle ||
      tag.ownerDocument!.defaultView!.getComputedStyle(tag as Element, '');
    let width = parseInt(tag.mlComputedStyle.width!),
      height = parseInt(tag.mlComputedStyle.height!);
  ...
  protected getParentBackground(
    tag: Element | PseudoElement,
    probeRect?: ClientRect,
  ) {
    let result: ColorEntry = Object.assign(
      {},
      tag.ownerDocument!.body.mlBgColor || NotFound,
    );
    ...
    if (tag.parentElement) {
      ...
      let isSvg =
        tag instanceof SVGElement && tag.parentElement instanceof SVGElement;
      tag.mlComputedStyle =
        tag.mlComputedStyle ||
        doc.defaultView!.getComputedStyle(tag as HTMLElement, '');

      if (
        tag.mlComputedStyle &&
        tag instanceof HTMLElement &&
        (tag.mlComputedStyle!.position == this._css.absolute ||
          tag.mlComputedStyle!.position == this._css.relative ||
          isSvg)
      ) {
        tag.zIndex = isSvg
          ? this.getElementIndex(tag)
          : parseInt(tag.mlComputedStyle!.zIndex || '0');
        tag.zIndex = isNaN(tag.zIndex!) ? -999 : tag.zIndex;
        let children: Element[] = Array.prototype.filter.call(
          tag.parentElement!.children,
          (otherTag: Element, index: number) => {
            if (otherTag != tag) {
              otherTag.zIndex =
                otherTag.zIndex || isSvg
                  ? -index
                  : parseInt(
                      (otherTag.mlComputedStyle = otherTag.mlComputedStyle
                        ? otherTag.mlComputedStyle
                        : doc.defaultView!.getComputedStyle(otherTag, ''))
                        .zIndex || '0',
                    );
              otherTag.zIndex = isNaN(otherTag.zIndex) ? -999 : otherTag.zIndex;
              if (
                otherTag.mlBgColor &&
                otherTag.mlBgColor.color &&
                otherTag.zIndex < tag.zIndex! &&
                otherTag.mlBgColor.role !== cc.TextSelection
              ) {
                probeRect =
                  probeRect ||
                  (tag.mlRect = tag.mlRect || tag.getBoundingClientRect());
                otherTag.mlRect =
                  otherTag.mlRect || otherTag.getBoundingClientRect();
                if (
                  otherTag.mlRect.left <= probeRect.left &&
                  otherTag.mlRect.top <= probeRect.top &&
                  otherTag.mlRect.right >= probeRect.right &&
                  otherTag.mlRect.bottom >= probeRect.bottom
                ) {
                  return true;
                }
              }
      ...
      if (bgColor && bgColor.alpha > 0.2) {
        ...
        probeRect =
          probeRect || (tag.mlRect = tag.mlRect || tag.getBoundingClientRect());
  ...
  protected restoreElementColors(
    tag: HTMLElement,
    ...
    if (
      tag.mlBgColor ||
      tag.mlInvert ||
      tag.originalBackgroundImage !== undefined ||
      ...
      if (
        tag.originalTransitionProperty !== undefined &&
        !keepTransition &&
      ...
      if (
        ...
        if (
          ...
          const { hasForbiddenTransition, newTransProps } =
            this.calculateTransition(tag.mlComputedStyle.transitionProperty);
          if (hasForbiddenTransition) {
            tag.originalTransitionProperty = tag.style.transitionProperty;
            tag.style.setProperty(
              this._css.transitionProperty,
              newTransProps.join(', '),
              this._css.important,
            );
          }
      ...
      if (
        tag.originalBackgroundColor !== undefined &&
        tag.originalBackgroundColor !==
          tag.style.getPropertyValue(ns.css.bgrColor)
      ) {
        tag.style.setProperty(ns.css.bgrColor, tag.originalBackgroundColor);
      }
      if (
        tag.originalDisplay !== undefined &&
      ...
      if (
        tag.originalTextShadow !== undefined &&
        tag.style.textShadow !== tag.originalTextShadow
      ) {
        tag.style.textShadow = tag.originalTextShadow!;
      }
      if (
        tag.originalBorderColor !== undefined &&
  ...
  protected calculateFilterRoomRules(tag: HTMLElement) {
    if (
      ...
      (!tag.mlComputedStyle ||
        tag.mlComputedStyle.getPropertyValue(this._css.mlIgnoreVar) !==
          true.toString())
    ) {
      ...
      if (tag.mlComputedStyle!.filter === this._css.none && tag.mlInvert) {
        hasRoomRules = true;
        roomRules.attributes =
          roomRules.attributes || new Map<string, string>();
  ...
  protected calculateSimplifiedRoomRules(tag: HTMLElement) {
    if (
        ...
        isButton =
          ...
          (tag instanceof HTMLInputElement &&
            (tag.type === 'button' ||
              ...
              tag.type === 'reset')) ||
          (tag instanceof Element &&
            tag.getAttribute(this._css.role) === 'button');
      ...
      if (
        !(tag instanceof HTMLImageElement) &&
        tag.mlComputedStyle!.backgroundImage &&
        tag.mlComputedStyle!.backgroundImage !== this._css.none &&
        tag.mlComputedStyle!.backgroundImage !== this._rootImageUrl
      ) {
        hasRoomRules = true;
        this.processBackgroundImagesAndGradients(
          tag,
          doc,
          roomRules,
          isButton,
  ...
  protected calculateRoomRules(tag: HTMLElement | PseudoElement):
    ...
    if (
      ...
      let isSvg = tag instanceof SVGElement,
        ...
        isButton =
          ...
          (!!tag.className &&
            ...
            /button\b|btn\b/gi.test(tag.className)),
        isTable =
      ...
      let beforePseudoElement: PseudoElement | undefined,
        afterPseudoElement: PseudoElement | undefined;

      tag instanceof HTMLElement &&
        tag.isEditableContent === undefined &&
      ...
      if (!isButton && tag instanceof HTMLLabelElement && tag.htmlFor) {
        ...
        isButton =
          labeledElement instanceof HTMLInputElement &&
      ...
      if (tag.mlComputedStyle) {
        ...
        if (tag instanceof HTMLElement) {
          ...
          if (
            ...
            beforeStyle.getPropertyValue(this._css.mlIgnoreVar) !==
              true.toString()
          ) {
            beforePseudoElement = new PseudoElement(
              PseudoType.Before,
              tag,
              beforeStyle,
            );
          ...
          if (
            ...
            afterStyle.getPropertyValue(this._css.mlIgnoreVar) !==
              true.toString()
          ) {
            afterPseudoElement = new PseudoElement(
              PseudoType.After,
              tag,
              afterStyle,
            );
        ...
        if (
          ...
          let { hasForbiddenTransition, newTransProps } =
            this.calculateTransition(tag.mlComputedStyle.transitionProperty);
          if (hasForbiddenTransition) {
        ...
        if (!isSvgText) {
          if (tag instanceof SVGElement) {
            if (
              this.tagIsSmall(
                tag instanceof SVGSVGElement ? tag : tag.ownerSVGElement || tag,
              ) &&
              tag.mlComputedStyle!.getPropertyValue(
                '--ml-small-svg-is-text',
              ) === true.toString()
            ) {
              isSvgText = true;
              roomRules.backgroundColor = Object.assign(
                {},
                this.getParentBackground(tag.ownerSVGElement || tag),
              );
              ...
              roomRules.backgroundColor = this.changeColor({
                role: cc.SvgBackground,
                property: ns.css.bgrColor,
                tag: tag,
                propVal: bgrColor,
              });
            }
            ...
            roomRules.backgroundColor = this.changeColor({
              role: isButton ? cc.ButtonBackground : cc.Background,
              property: ns.css.bgrColor,
              tag: tag,
              propVal: bgrColor,
            });
          ...
          if (
            this._app.preserveDisplay &&
            roomRules.backgroundColor &&
            roomRules.backgroundColor.color &&
            tag.id &&
            tag.className
          ) {
        ...
        if (
          tag.tagName == ns.img &&
          (this.shift.Image.lightnessLimit < 1 ||
            this.shift.Image.saturationLimit < 1 ||
            this._settingsManager.currentSettings.blueFilter !== 0)
        ) {
          const customImageRole = tag.mlComputedStyle!.getPropertyValue(
            `--ml-${cc[cc.Image].toLowerCase()}`,
          ) as keyof ComponentShift;
          let imgSet = this.shift[customImageRole] || this.shift.Image;
          roomRules.filter = {
            value: [
              tag.mlComputedStyle!.filter != this._css.none
                ? tag.mlComputedStyle!.filter
                : '',
              imgSet.saturationLimit < 1
                ? `saturate(${imgSet.saturationLimit})`
                : '',
              this._settingsManager.currentSettings.blueFilter !== 0
                ? `var(--${FilterType.BlueFilter})`
                : '',
              imgSet.lightnessLimit < 1
                ? `brightness(${imgSet.lightnessLimit})`
                : '',
            ]
              .filter((f) => f)
              .join(' ')
        ...
        let bgInverted =
          roomRules.backgroundColor!.originalLight -
            roomRules.backgroundColor!.light >
          0.4;
        if (
          roomRules.backgroundColor!.color &&
          roomRules.backgroundColor!.alpha < 0.2
        ) {
        ...
        if (
          tag.mlComputedStyle!.content!.startsWith('url') &&
          !(
            tag instanceof PseudoElement &&
            tag.parentElement.mlComputedStyle!.content ===
              tag.mlComputedStyle!.content
          )
        ) {
          let doInvert =
            !isTable &&
            ...
            tag.mlComputedStyle!.getPropertyValue('--ml-no-invert') !==
              true.toString() &&
            (this.tagIsSmall(tag) ||
              (tag.isPseudo &&
                tag.parentElement!.parentElement &&
                this.tagIsSmall(tag.parentElement!.parentElement!) &&
                tag.parentElement!.parentElement!.mlComputedStyle!.overflow ===
                  this._css.hidden) ||
              (!tag.isPseudo &&
                this.tagIsSmall(tag.parentElement!) &&
                tag.parentElement!.mlComputedStyle!.overflow ===
                  this._css.hidden));
          if (
            this.shift.Image.lightnessLimit < 1 ||
            this.shift.Image.saturationLimit < 1 ||
            ...
            roomRules.filter = {
              value: [
                ...
                imgSet.saturationLimit < 1
                  ? `saturate(${imgSet.saturationLimit})`
                  : '',
                imgSet.lightnessLimit < 1 && !doInvert
                  ? `brightness(${imgSet.lightnessLimit})`
                  : '',
                doInvert
                  ? `brightness(${float.format(
        ...
        if (!isSvg || isSvgText) {
          if (
            ...
            (!isSvg && //tag.isPseudo &&
              (tag.parentElement instanceof HTMLAnchorElement ||
                (tag.parentElement &&
                  tag.parentElement.mlColor &&
                  tag.parentElement.mlColor.role === cc.Link)))
          ) {
            const linkColor = this.changeColor({
              role: cc.Link,
              property: ns.css.fntColor,
              tag: tag,
              bgLight: bgLight,
            ...
            if (
              tag instanceof HTMLFontElement ||
              (linkColor && linkColor.role !== cc.Link)
            ) {
              roomRules.color = linkColor;
            } else if (linkColor) {
              roomRules.linkColor = linkColor;
              roomRules.linkColor$Avtive = this.changeColor({
                role: cc.Link$Active,
                propVal: linkColor.originalColor,
                property: ns.css.fntColor,
                tag: tag,
                bgLight: bgLight,
              });
              roomRules.linkColor$Hover = this.changeColor({
                role: cc.Link$Hover,
                propVal: linkColor.originalColor,
                property: ns.css.fntColor,
                tag: tag,
              ...
              roomRules.visitedColor = this.changeColor({
                role: cc.VisitedLink,
                propVal: linkColor.originalColor,
                property: ns.css.fntColor,
                tag: tag,
              ...
              roomRules.visitedColor$Active = this.changeColor({
                role: cc.VisitedLink$Active,
                propVal: linkColor.originalColor,
                property: ns.css.fntColor,
                tag: tag,
                bgLight: bgLight,
              });
              roomRules.visitedColor$Hover = this.changeColor({
                role: cc.VisitedLink$Hover,
                propVal: linkColor.originalColor,
                property: ns.css.fntColor,
                tag: tag,
                bgLight: bgLight,
            ...
            if (
              ...
              roomRules.color = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
              ...
              roomRules.color = this.changeColor({
                role:
                  roomRules.backgroundColor!.role === cc.HighlightedBackground
                    ? cc.HighlightedText
                    : cc.Text,
                property: ns.css.fntColor,
                tag: tag,
          ...
          if (roomRules.color || roomRules.linkColor) {
            ...
            let originalTextContrast = Math.abs(
              roomRules.backgroundColor!.originalLight -
                textColor.originalLight,
            );
            let currentTextContrast = Math.abs(
              roomRules.backgroundColor!.light - textColor.light,
            );
            if (
              currentTextContrast != originalTextContrast &&
              textColor.originalLight != textColor.light &&
              tag.mlComputedStyle!.textShadow &&
              tag.mlComputedStyle!.textShadow !== this._css.none
            ) {
              let newTextShadow = tag.mlComputedStyle!.textShadow!,
                newColor: ColorEntry | undefined = undefined,
                currentTextShadowColor: string | null,
                prevHslColor: HslaColor,
                shadowContrast: number,
                inheritedShadowColor;
              let uniqColors = new Set<string>(
                newTextShadow
                  .replace(/([\.\d]+px)/gi, '')
              ...
              if (uniqColors.size > 0) {
                uniqColors.forEach((c) => {
                  currentTextShadowColor = /rgb/gi.test(c)
                    ? c
                    : this._colorConverter.convert(c);
                  if (currentTextShadowColor) {
                    newColor = this.changeColor({
                      role: cc.TextShadow,
                      property: ns.css.shdColor,
                      bgLight: textColor.light,
                      propVal: currentTextShadowColor,
                      tag,
                    });
                    if (newColor && newColor.color) {
                      newTextShadow = newTextShadow.replace(
        ...
        if (
          (isSvg && tag.mlComputedStyle!.stroke !== this._css.none) ||
          (!isSvg &&
            ((tag.mlComputedStyle!.borderStyle &&
              tag.mlComputedStyle!.borderStyle !== this._css.none) ||
              (!tag.mlComputedStyle!.borderStyle &&
          ...
          const brdColorIsSingle =
            (brdColor && brdColor.indexOf(' r') === -1) ||
            (!brdColor &&
          ...
          if (brdColorIsSingle) {
            ...
            if (brdColor === bgrColor) {
              roomRules.borderColor = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
                  reason: ColorReason.SameAsBackground,
                  owner: this._app.isDebug ? tag : null,
              ...
              roomRules.borderColor = this.changeColor({
                role: isButton ? cc.ButtonBorder : cc.Border,
                property: ns.css.brdColor,
                tag: tag,
                bgLight: bgLight,
                propVal: brdColor,
              });
            ...
            let borderRole = isButton ? cc.ButtonBorder : cc.Border,
              transBordersCount = 0;
            if (
              tag.isPseudo &&
              tag.mlComputedStyle!.width === this._css._0px &&
              tag.mlComputedStyle!.height === this._css._0px &&
              ((transBordersCount = [
                tag.mlComputedStyle!.borderTopColor,
                tag.mlComputedStyle!.borderRightColor,
                tag.mlComputedStyle!.borderBottomColor,
            ...
            if (tag.mlComputedStyle!.borderTopColor === bgrColor) {
              roomRules.borderTopColor = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
                  reason: ColorReason.SameAsBackground,
                  owner: this._app.isDebug ? tag : null,
              ...
              roomRules.borderTopColor = this.changeColor({
                role: borderRole,
                property: this._css.borderTopColor,
                tag: tag,
                bgLight: bgLight,
              });
            }
            ...
            if (tag.mlComputedStyle!.borderRightColor === bgrColor) {
              roomRules.borderRightColor = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
                  reason: ColorReason.SameAsBackground,
                  owner: this._app.isDebug ? tag : null,
              ...
              roomRules.borderRightColor = this.changeColor({
                role: borderRole,
                property: this._css.borderRightColor,
                tag: tag,
                bgLight: bgLight,
              });
            }
            ...
            if (tag.mlComputedStyle!.borderBottomColor === bgrColor) {
              roomRules.borderBottomColor = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
                  reason: ColorReason.SameAsBackground,
                  owner: this._app.isDebug ? tag : null,
              ...
              roomRules.borderBottomColor = this.changeColor({
                role: borderRole,
                property: this._css.borderBottomColor,
                tag: tag,
                bgLight: bgLight,
              });
            }
            ...
            if (tag.mlComputedStyle!.borderLeftColor === bgrColor) {
              roomRules.borderLeftColor = Object.assign(
                Object.assign({}, roomRules.backgroundColor),
                {
                  reason: ColorReason.SameAsBackground,
                  owner: this._app.isDebug ? tag : null,
              ...
              roomRules.borderLeftColor = this.changeColor({
                role: borderRole,
                property: this._css.borderLeftColor,
                tag: tag,
                bgLight: bgLight,
              });
            }
      ...
      return {
        roomRules: roomRules,
        before: beforePseudoElement,
        after: afterPseudoElement,
      };
    }
    return undefined;
  }
  ...
  private processInaccessibleTextContent({
    ...
    strictBgLight = false,
    customTextLight,
    ignoreContentInvertRule = false,
  }: {
    tag: HTMLCanvasElement | HTMLEmbedElement;
    ignoreContentInvertRule?: boolean;
    roomRules: RoomRules;
    strictBgLight?: boolean;
    customTextLight?: number;
  }) {
    ...
    if (
      ...
      const darkSet = this.shift.Background,
      ...
      filterValue = [
        ...
        darkSet.saturationLimit < 1
          ? `saturate(${darkSet.saturationLimit})`
          : '',
        `brightness(${float.format(
          Math.max(1 - darkSet.lightnessLimit, strictBgLight ? 0 : 0.9),
        )})`,
        `hue-rotate(180deg) invert(1)`,
        applyContentFilter ? `var(--${FilterType.ContentFilter})` : '',
        `brightness(${float.format(
          Math.max(customTextLight || txtSet.lightnessLimit, 0.9),
        )})`,
      ];
    } else {
      const lightSet = this.shift.Image;
      filterValue = [
        lightSet.saturationLimit < 1
          ? `saturate(${lightSet.saturationLimit})`
          : '',
        applyContentFilter ? `var(--${FilterType.ContentFilter})` : '',
        lightSet.lightnessLimit < 1
          ? `brightness(${lightSet.lightnessLimit})`
          : '',
      ];
    }
    roomRules.filter = {
  ...
  protected changeColor({
    ...
    if (tag.mlComputedStyle) {
      let propRole = (cc as any as { [p: string]: Component })[
        tag.mlComputedStyle.getPropertyValue(
          `--ml-${cc[component].replace('$', '-').toLowerCase()}-${property}`,
        )
      ];
      ...
      switch (propRole) {
        case cc.Background:
          return this._backgroundColorProcessor.changeColor(
            propVal,
            true,
            tag,
            this._boundParentBackgroundGetter,
          );
        ...
        case cc.ButtonBackground:
          ...
          return this._buttonBackgroundColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.HighlightedBackground:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._highlightedBackgroundColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.TextShadow:
          return this._textShadowColorProcessor.changeColor(
            propVal,
            bgLight!,
            tag,
          );
        ...
        case cc.HighlightedText:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._highlightedTextColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.Link$Active:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._activeLinkColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.Link$Hover:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._hoverLinkColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.VisitedLink:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._visitedLinkColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.VisitedLink$Active:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._activeVisitedLinkColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.VisitedLink$Hover:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._hoverVisitedLinkColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.Border:
          bgLightVal =
            bgLight !== undefined
              ? bgLight
              : this.getParentBackground(tag).light;
          return this._borderColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.ButtonBorder:
          bgLightVal =
            /*bgLight !== undefined ? bgLight :*/ this.getParentBackground(
              tag,
            ).light;
          return this._buttonBorderColorProcessor.changeColor(
            propVal,
            bgLightVal,
            tag,
          );
        ...
        case cc.SvgBackground:
          return this._svgColorProcessor.changeColor(
            propVal,
            false,
            tag,
            this._boundParentBackgroundGetter,
        ...
        case cc.TextSelection:
          return this._textSelectionColorProcessor.changeColor(
            propVal,
            false,
            tag,
            this._boundParentBackgroundGetter,
  ...
  protected processBackgroundImagesAndGradients(
    ...
    if (/\burl\(/gi.test(backgroundImage)) {
      const customBgImageRole = tag.mlComputedStyle!.getPropertyValue(
        `--ml-${cc[cc.BackgroundImage].toLowerCase()}`,
      ) as keyof ComponentShift;
      const bgImgSet =
        this.shift[customBgImageRole] || this.shift.BackgroundImage;
      const hasRepeats =
        !/^(?:,?\s?no-repeat)+$/i.test(
          tag.mlComputedStyle!.backgroundRepeat!,
        ) &&
        !/cover|contain|%|^(?:,?\s?\d+\dpx)+$/i.test(
          tag.mlComputedStyle!.backgroundSize!,
        ) &&
      ...
      if (
        bgImgSet.lightnessLimit < 1 ||
        bgImgSet.saturationLimit < 1 ||
        doInvert ||
        this._settingsManager.currentSettings.blueFilter !== 0 ||
        ...
        if (bgImgSet.lightnessLimit < 1 && !doInvert && !hasRepeats) {
          ...
          const area =
              1 - Math.min(Math.max(tag.mlArea!, 1) / doc.viewArea!, 1),
            lim = bgImgSet.lightnessLimit,
            txtLim = this.shift.Text.lightnessLimit;
          bgImgLight = Math.min(
            ((lim ** (1 / 2) - lim) ** (1 / 3) * area) ** 3 + lim,
            Math.max(lim, txtLim),
          );
        } else if (hasRepeats) {
        ...
        bgFilter = [
          bgImgSet.saturationLimit < 1
            ? `saturate(${bgImgSet.saturationLimit})`
            : '',
          bgImgLight < 1 && !doInvert
            ? `brightness(${float.format(bgImgLight)})`
            : '',
          doInvert
            ? `brightness(${float.format(
                1 - this.shift.Background.lightnessLimit,
        ...
        haveToProcBgImg =
          (tag instanceof Element && !!tag.firstChild) ||
          isPseudoContent ||
          haveToProcBgGrad ||
          (roomRules.backgroundColor && !!roomRules.backgroundColor.color) ||
          tag instanceof HTMLInputElement ||
          tag instanceof HTMLTextAreaElement ||
          tag instanceof HTMLBodyElement ||
          tag instanceof HTMLHtmlElement;
    ...
    if (haveToProcBgImg || haveToProcBgGrad) {
      ...
      if (
        (roomRules.hasBackgroundImageSet = /image-set\(/gi.test(
          backgroundImage,
        ))
      ) {
      ...
      const gradientColorMatches = backgroundImage.match(
        /rgba?\([^)]+\)|(color-stop|from|to)\((rgba?\([^)]+\)|[^)]+)\)|calc\([^)]+\)/gi,
      );
      const gradientColors = new Map<string, string>();
      if (gradientColorMatches) {
        gradientColorMatches.forEach((color) =>
          gradientColors.set(color, Math.random().toString()),
        );
        gradientColors.forEach(
          (id, color) =>
            (backgroundImage = backgroundImage.replace(
              new RegExp(escapeRegex(color), 'g'),
              id,
            )),
        );
      }

      const backgroundImages = backgroundImage.match(
        /\burl\(\"[^"]+\"\)|[\w-]+\([^)]+\)/gi,
      )!;

      roomRules.backgroundImages = backgroundImages.map((bgImg, index) => {
        gradientColors.forEach(
          (id, color) => (bgImg = bgImg.replace(new RegExp(id, 'g'), color)),
        );
        if (haveToProcBgImg && bgImg.startsWith('url')) {
          return this._backgroundImageProcessor.process(
            bgImg,
            bgFilter,
            this._settingsManager.currentSettings.blueFilter / 100,
            roomRules,
          );
        } else if (
          !this._settingsManager.isFilter &&
          /gradient/gi.test(bgImg)
        ) {
          return this.processBackgroundGradient(
            tag,
            isButton,
            index,
            bgImg,
  ...
  protected processBackgroundGradient(
    tag: HTMLElement | PseudoElement,
    isButton: boolean,
    index: number,
    gradient: string,
    ...
    let mainColor: ColorEntry | null = null;
    let lightSum = 0;

    // Extract unique color values from the gradient string
    const uniqColors = new Set<string>(
      gradient
        .replace(
          /webkit|moz|ms|repeating|linear|radial|from|\bto\b|gradient|circle|ellipse|top|left|bottom|right|farthest|closest|side|corner|current|color|transparent|stop|calc|[\.\d]+%|[\.\d]+[a-z]{2,4}/gi,
          '',
        )
        .match(/(rgba?\([^\)]+\)|#[a-z\d]{6}|[a-z]+)/gi) || [],
    );

    // Determine the background lightness limit
    ...
    if (uniqColors.size > 0) {
      uniqColors.forEach((c) => {
        ...
        const prevColor = /rgb/i.test(c) ? c : this._colorConverter.convert(c);
        let newColor: ColorEntry;

        if (isButton) {
          newColor = this.changeColor({
            role: cc.ButtonBackground,
            property: this._css.backgroundColor,
            tag: tag,
            propVal: prevColor!,
            bgLight: bgLight,
          })!;
        ...
        // Replace the original color in the gradient with the new color
        if (
          newColor.color ||
          (newColor.reason === ColorReason.Inherited && newColor.intendedColor)
        ) {
          gradient = gradient.replace(
            new RegExp(this.escapeRegex(c), 'gi'),
            newColor.color || newColor.intendedColor!,
          );
        }

        // Assign mainColor based on certain conditions
        if (
          !mainColor &&
          newColor.alpha > 0.5 &&
          roomRules.backgroundColor &&
          newColor.role ===
            (isButton ? cc.ButtonBackground : cc.BackgroundNoContrast)
        ) {
          // **Refactored Assignment with Explicit Type Annotation**
          const updatedBackgroundColor: ColorEntry = {
            ...roomRules.backgroundColor,
            light: newColor.light, // Initialize with the new light value
          };

          roomRules.backgroundColor = updatedBackgroundColor;
          mainColor = updatedBackgroundColor;
          mainColor.light = 0;
        }
      });

      if (mainColor) {
        // **Ensure mainColor is of type ColorEntry**
        (mainColor as ColorEntry).light = lightSum / uniqColors.size;
      }
    }
  ...
  protected applyBackgroundImages(
    tag: HTMLElement | PseudoElement,
    hasBackgroundImageSet: boolean | undefined,
    backgroundImages: BackgroundImage[],
  ) {
    let originalState = this._documentObserver.stopDocumentObservation(
      tag.ownerDocument!,
    );
    let bgImages = backgroundImages
      .map(
        (bgImg, ix) =>
          bgImg.data + (hasBackgroundImageSet ? ` ${ix + 1}x` : ''),
      )
      .join(',');
  ...
  protected getStandardPseudoStyles() {
    ...
    for (let pseudoType of getEnumNames(PseudoType)) {
      for (let pseudoStandard of getEnumValues<PseudoStyleStandard>(
        ...
        const standard = PseudoStyleStandard[pseudoStandard]
          .replace(/(\B[A-Z])/g, '-$1')
          .toLowerCase();
        this._pseudoStyles.set(pseudo + cssText, standard);
        css.push(
          `[${pseudo}-style="${standard}"]:not(imp)::${pseudo}{${cssText}}`,
        );
      }
    }
    return css.join('\n');
  ...
  protected calculateMainColors(doc: Document) {
    ...
    const bgLight = this.shift.Background.lightnessLimit,
      textColorEntry = this._textColorProcessor.changeColor(
        cx.Black,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
    ...
    let backgroundColor = this._backgroundColorProcessor.changeColor(
        cx.White,
        true,
        doc.documentElement,
        undefined,
      ...
      altBackgroundColor = this._backgroundColorProcessor.changeColor(
        'rgb(250,250,250)',
        true,
        doc.documentElement,
        undefined,
      ...
      transBackgroundColor = this._backgroundColorProcessor.changeColor(
      ...
      transAltBackgroundColor = this._backgroundColorProcessor.changeColor(
      ...
      transTextColor = this._textColorProcessor.changeColor(
        'rgba(0,0,0,0.6)',
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      textColorFiltered = this._textColorProcessor.changeColor(
        cx.Black,
        bgLight,
        doc.documentElement,
      ).color!,
      borderColor = this._borderColorProcessor.changeColor(
        cx.Gray,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      transBorderColor = this._borderColorProcessor.changeColor(
        'rgba(127,127,127,0.3)',
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      selectionColor = this._textSelectionColorProcessor.changeColor(
        cx.White,
        false,
        doc.documentElement,
        undefined,
      ...
      rangeFillColor = this._rangeFillColorProcessor.changeColor(
        this.shift,
        textColorEntry.light,
        bgLight,
        ignoreBlueFilter,
      ).color!,
      buttonBackgroundColor = this._buttonBackgroundColorProcessor.changeColor(
        cx.White,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ...
      redButtonBackgroundColor =
        this._buttonBackgroundColorProcessor.changeColor(
          'rgb(255,0,0)',
          bgLight,
          doc.documentElement,
          ignoreBlueFilter,
        ).color!,
      buttonBorderColor = this._buttonBorderColorProcessor.changeColor(
        cx.White,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      scrollbarThumbHoverColor = this._scrollbarHoverColorProcessor.changeColor(
        cx.White,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      scrollbarThumbNormalColor =
        this._scrollbarNormalColorProcessor.changeColor(
          cx.White,
          bgLight,
          doc.documentElement,
          ignoreBlueFilter,
      ...
      scrollbarThumbActiveColor =
        this._scrollbarActiveColorProcessor.changeColor(
          cx.White,
          bgLight,
          doc.documentElement,
          ignoreBlueFilter,
      ...
      scrollbarThumbHoverColorFiltered =
        this._scrollbarHoverColorProcessor.changeColor(
          cx.White,
          bgLight,
          doc.documentElement,
        ).color!,
      ...
      scrollbarTrackColorFiltered = this._backgroundColorProcessor.changeColor(
        cx.White,
        false,
        doc.documentElement,
      ).color!,
      linkColor = this._linkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      linkColorHover = this._hoverLinkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      linkColorActive = this._activeLinkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      visitedColor = this._visitedLinkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      visitedColorHover = this._hoverVisitedLinkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
      ).color!,
      visitedColorActive = this._activeVisitedLinkColorProcessor.changeColor(
        cx.Link,
        bgLight,
        doc.documentElement,
        ignoreBlueFilter,
    ...
    const backgroundColorFiltered = this._backgroundColorProcessor.changeColor(
        cx.White,
        true,
        doc.documentElement,
      ).color!,
      altBackgroundColorFiltered = this._backgroundColorProcessor.changeColor(
        'rgb(250,250,250)',
        true,
        doc.documentElement,
      ).color!;
    ...
    const mainColors = {
      backgroundColor,
      backgroundColorFiltered,
      altBackgroundColor,
      altBackgroundColorFiltered,
      transBackgroundColor,
      transAltBackgroundColor,
      textColor,
      transTextColor,
      textColorFiltered,
      borderColor,
      ...
      scrollbarSize,
      mozScrollbarWidth,
      mozScrollbarTrackColor: altBackgroundColor,
      linkColor,
      linkColorHover,
      linkColorActive,
      visitedColor,
  ...
  protected injectDynamicValues(doc: Document) {
    ...
    this._svgFilters.createSvgFilters(
      doc,
    ...
    let component: keyof ComponentShift, property: keyof ColorShift;
    for (component in this.shift) {
      for (property in this.shift[component]) {
        cssText += `\n--ml${component
          .replace('$', '')
          .replace(/([A-Z])/g, '-$1')}-${property.replace(/([A-Z])/g, '-$1')}:${
    ...
    cssText += `\n--ml-invert:${
      this.shift.Background.lightnessLimit < 0.3 ? 1 : 0
    }!important;`;
    cssText += `\n--ml-is-active:${
      this._settingsManager.isActive ? 1 : 0
    }!important;`;

    const fakeCanvas = doc.createElement('canvas'),
      fakeCanvasRules: RoomRules = {};
    ...
    for (const [contentType, ignoreContentInvertRule] of [
      ...
      this.processInaccessibleTextContent({
        tag: fakeCanvas,
        roomRules: fakeCanvasRules,
        strictBgLight: true,
        ignoreContentInvertRule,
      });
      cssText += `\n--ml${contentType}-text-filter:${
        fakeCanvasRules.filter!.value
      };`;
      ...
      cssText += `\n--ml${contentType}-contrast-text-filter:${
        fakeCanvasRules.filter!.value
      };`;
      ...
      cssText += `\n--ml${contentType}-highlighted-text-filter:${
        fakeCanvasRules.filter!.value
      };`;
    }
    ...
    const bgImgRevertFilter = this.GetComponentRevertFilter(
      this.shift.BackgroundImage,
      invertColors,
    );
    cssText += `\n--ml-bg-image-revert-filter:${bgImgRevertFilter || 'none'};`;

    const buttonRevertFilter = this.GetComponentRevertFilter(
      this.shift.BackgroundImage,
      invertColors,
    );
    cssText += `\n--ml-button-revert-filter:${buttonRevertFilter || 'none'};`;

    const textRevertFilter = this.GetComponentRevertFilter(
      this.shift.Text,
      invertColors,
    );
    cssText += `\n--ml-text-revert-filter:${textRevertFilter || 'none'};`;

    const videoRevertFilter = this.GetComponentRevertFilter(
      this.shift.Video,
      invertColors,
    );
    cssText += `\n--ml-video-revert-filter:${videoRevertFilter || 'none'};`;

    const bgRevertFilter = this.GetComponentRevertFilter(
  ...
  private GetComponentRevertFilter(
    cmpShift: ColorShift,
    invertComponent: boolean,
  ) {
    let cmpLight =
        cmpShift.lightnessLimit +
        1 -
        Math.max(this.shift.Text.lightnessLimit, 0.9),
      cmpSat =
        cmpShift.saturationLimit + 1 - this.shift.Background.saturationLimit;
  ...
  protected processMetaTheme(doc: Document) {
    if (doc.head && this._settingsManager.currentSettings.changeBrowserTheme) {
      let metaTheme = doc.querySelector(
      ...
      if (!metaTheme) {
        ...
        originalColor =
          this._colorConverter.convert(metaTheme.content) || originalColor;
  ...
  public applyRoomRules(
    ...
    if (tag instanceof Element) {
      if (roomRules.attributes && roomRules.attributes.size > 0) {
        roomRules.attributes.forEach((attrValue, attrName) =>
    ...
    if (roomRules.transitionProperty && roomRules.transitionProperty.value) {
      ...
      tag.style.setProperty(
        this._css.transitionProperty,
        roomRules.transitionProperty.value,
        this._css.important,
      );
    }
    ...
    if (roomRules.backgroundImages) {
      ...
      if (roomRules.hasBackgroundImagePromises) {
        applyBgPromise = Promise.all([
          tag,
          tag.mlTimestamp,
          roomRules.hasBackgroundImageSet,
          ...roomRules.backgroundImages,
        ] as [
        ...
        this.applyBackgroundImages(
          tag,
          roomRules.hasBackgroundImageSet,
          roomRules.backgroundImages as BackgroundImage[],
        );
      }
    }
    ...
    if (roomRules.backgroundColor && roomRules.backgroundColor.color) {
      if (
        tag instanceof HTMLElement &&
        (tag.isEditableContent ||
          (tag.isEditableContent === undefined && this.isEditableContent(tag)))
      ) {
        tag.style.setProperty(
          this._css.editableContentBackgroundColor,
          roomRules.backgroundColor.color,
        );
      } else {
        tag.originalBackgroundColor = tag.style.getPropertyValue(
          ns.css.bgrColor,
        );
        tag.style.setProperty(
          ns.css.bgrColor,
    ...
    if (roomRules.placeholderColor && roomRules.placeholderColor.color) {
      tag.style.setProperty(
        this._css.placeholderColor,
        roomRules.placeholderColor.color,
        this._css.important,
      );
    }
    ...
    if (
      ...
      if (
        ...
        tag.style.setProperty(
          this._css.editableContentColor,
          roomRules.color.color ||
            roomRules.color.intendedColor ||
            roomRules.color.inheritedColor ||
            cx.Black,
        );
      } else {
      ...
      roomRules.color &&
      roomRules.color.reason === ColorReason.Inherited &&
      tag.style.getPropertyValue(ns.css.fntColor)
    ) {
      tag.originalColor = '';
    }
    ...
    if (roomRules.linkColor && roomRules.linkColor.color) {
      tag.style.setProperty(
        this._css.linkColor,
        roomRules.linkColor.color,
        this._css.important,
      );
      tag.style.setProperty(
        this._css.linkColorHover,
        roomRules.linkColor$Hover!.color,
        this._css.important,
      );
      tag.style.setProperty(
        this._css.linkColorActive,
        roomRules.linkColor$Avtive!.color,
        this._css.important,
      );
    }
    ...
    if (roomRules.visitedColor && roomRules.visitedColor.color) {
      tag.style.setProperty(
        this._css.visitedColor,
        roomRules.visitedColor.color,
        this._css.important,
      );
      tag.style.setProperty(
        this._css.visitedColorHover,
        roomRules.visitedColor$Hover!.color,
        this._css.important,
      );
      tag.style.setProperty(
        this._css.visitedColorActive,
        roomRules.visitedColor$Active!.color,
        this._css.important,
      );
    }
    ...
    if (
      ...
      tag.style.setProperty(
        this._css.originalBackgroundColor,
        tag.originalBackgroundColor ||
          (roomRules.backgroundColor &&
            roomRules.backgroundColor.originalColor !== cx.Transparent &&
            roomRules.backgroundColor.originalColor) ||
          this.getParentBackground(tag).originalColor ||
          cx.White,
      );
    }
    ...
    if (roomRules.borderColor && roomRules.borderColor.color) {
      ...
      if (roomRules.borderTopColor && roomRules.borderTopColor.color) {
        ...
        tag.style.setProperty(
          this._css.borderTopColor,
          roomRules.borderTopColor.color,
          this._css.important,
        );
      }
      ...
      if (roomRules.borderRightColor && roomRules.borderRightColor.color) {
        ...
        tag.style.setProperty(
          this._css.borderRightColor,
          roomRules.borderRightColor.color,
          this._css.important,
        );
      }
      ...
      if (roomRules.borderBottomColor && roomRules.borderBottomColor.color) {
        ...
        tag.style.setProperty(
          this._css.borderBottomColor,
          roomRules.borderBottomColor.color,
          this._css.important,
        );
      }
      ...
      if (roomRules.borderLeftColor && roomRules.borderLeftColor.color) {
        ...
        tag.style.setProperty(
          this._css.borderLeftColor,
          roomRules.borderLeftColor.color,
          this._css.important,
        );
      }
    }
    ...
    if (tag instanceof PseudoElement) {
      if (applyBgPromise) {
        ...
        Promise.all([tag, applyBgPromise.catch((ex) => ex)]).then(([t]) =>
          t.applyStyleChanges(),
        );
      } else {
        ...
        if (cssText) {
          ...
          tag.parentElement.setAttribute(
            `${tag.tagName}-style`,
            pseudoId || tag.id,
          );
        }
